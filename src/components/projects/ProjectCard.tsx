...
export function ProjectCard({ project, index }: ProjectCardProps) {
  return (
    <motion.div
      ...
      className="group"
    >
      <div className="relative overflow-hidden rounded-2xl mb-3 sm:mb-4 aspect-[16/9] bg-surface">
        <div className="absolute inset-0 bg-gradient-to-t from-accent/80 via-accent/20 to-transparent z-10" />
        <div className="absolute inset-0 flex items-center justify-center text-secondary">
          <svg className="w-12 sm:w-16 h-12 sm:h-16" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="space-y-2 sm:space-y-3">
        <Badge variant="primary" className="text-xs sm:text-sm">{project.category}</Badge>
